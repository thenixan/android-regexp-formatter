...
class WordRegExpItem(override val length: Length) : RegExpItem {
    ...
    override fun format(input: RegularExpressionSpannable, startPosition: Int, endPosition: Int) = (0 until endPosition - startPosition).takeWhile { length.compareWithPosition(it) <= 0 }.count()
